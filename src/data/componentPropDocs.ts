...
export const componentPropDocs: Record<ComponentSlug, PropDoc[]> = {
	...
	'gas-giant': [
		{ name: 'textureUrl', type: 'string', default: "'/jupiter/8k_jupiter.jpg'", description: 'Path to the equirectangular gas giant panorama.' },
		{ name: 'procedural', type: 'boolean', default: 'false', description: 'Enable procedural gas giant generation (fBm banding), instead of the texture asset.' },
		{ name: 'fallbackColor1', type: 'string', default: "'#222'", description: 'CSS gradient fallback colour (center).' },
		{ name: 'fallbackColor2', type: 'string', default: "'#000'", description: 'CSS gradient fallback colour (edge).' },
		{ name: 'atmosphereColor', type: 'string', default: "'#e9d8a6'", description: 'Tint colour for the atmospheric edge/terminator glow (Jupiter warm tones).' },
		{ name: 'atmosphereIntensity', type: 'number', default: '1.2', min: 0.0, max: 3.0, step: 0.1, description: 'Intensity multiplier for the atmospheric glow at the day/night terminator.' },
		{ name: 'speed', type: 'number', default: '0.015', min: 0.0, max: 0.5, step: 0.01, description: 'Rotation speed of the planet.' },
		{ name: 'tilt', type: 'number', default: '0.15', min: -1.57, max: 1.57, step: 0.05, description: 'Axial tilt in radians. Angles the poles.' },
		{ name: 'lightDirection', type: '[number, number, number]', default: '[1.0, 0.0, 0.6]', control: 'hidden', description: 'Direction vector of the primary light source.' },
		{ name: 'radius', type: 'number', default: '0.55', min: 0.2, max: 1.5, step: 0.05, description: 'Planet base radius (fraction of viewport height).' },
		{ name: 'oblateness', type: 'number', default: '1.06', min: 1.0, max: 1.3, step: 0.01, description: 'Scale factor to squash the planet at poles (oblateness). Jupiter is ~1.06.' },
		{ name: 'hasRings', type: 'boolean', default: 'false', description: 'Enable a procedural ring system (Saturn-style). Rings include a simulated Cassini division and cast shadows onto the planet.' },
		{ name: 'ringColor', type: 'string', default: "'#dbc8a0'", description: 'Base tint colour for the ring system.' },
		{ name: 'turbulence', type: 'number', default: '0.05', min: 0.0, max: 0.3, step: 0.01, description: 'Fractional Brownian Motion (fBm) warp multiplier. Simulates fluid atmospheric turbulence and storm vortices on the surface.' },
		{ name: 'starfield', type: 'boolean', default: 'true', description: 'Enable a procedural starfield background with parallax depth tied to mouse position.' },
		{ name: 'mouseReact', type: 'boolean', default: 'true', description: 'Enable mouse-reactive lighting: moving the cursor nudges the day/night terminator and viewport tilt.' },
		{ name: 'bandColor1', type: 'string', default: "'#3d2b1f'", description: 'Primary band color for procedural mode (when no textureUrl is set). Controls the darker cloud belt hue.' },
		{ name: 'bandColor2', type: 'string', default: "'#d4c3a1'", description: 'Secondary band color for procedural mode (when no textureUrl is set). Controls the lighter zone hue between bands.' },
		...wrapperProps,
	],
};
